...
for lo_idx = 1:n_lo_steps
    theta = []; % keep track of this to skip the right theta

    fprintf(repmat(char(8),1,length(msg)));
    msg = sprintf('Computing low - %.2f:%.2f high - %.2f:%.2f:%.2f', ...
        lo_freqs(1,lo_idx),lo_freqs(2,lo_idx),...
        hi_freqs(1,1,lo_idx),...
        hi_freqs(1,2,lo_idx)-hi_freqs(1,1,lo_idx),...
        hi_freqs(2,end,lo_idx));
    fprintf(msg);
