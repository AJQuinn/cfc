...
% Compute surrogates
nsurrogates = 200;
surrogate_Z = nan(nchannels,nsurrogates);
skips = randi([fix(nsamples/10),9*fix(nsamples/10)],nsurrogates,1);
for idx = 1:nsurrogates
    surrogate_Z(:,idx) = abs(mean( circshift(modulated_signal,skips(idx),2).*exp(1i * modulating_signal),2 ));
end

% Normalise
Z_length = (abs(mean_vector)-mean(surrogate_Z,2))./std(surrogate_Z,[],2);
