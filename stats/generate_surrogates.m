function surrogates = generate_surrogates( data, n, mean_term, std_term )
...
rand_spec = zeros(n,length(noise));
for idx = 1:n
    rand_spec(idx,:) = amp_spec.*exp(1i*noise(idx,:));
end
...
for idx = 1:n
    surrogates(idx,:) = surrogates(idx,:) / ( std(surrogates(idx,:),[],2) / std_term );
end
